...
<body>
    <div class="card"> {{-- css gonna identified by class name, so make sure i remember each class name --}}
        ...
        <p class="p">Masuk Sebagai Penyedia Kerja</p>

        <div class="card-body">

            <form id="form" action="{{ route('hit-login-perusahaan') }}" method="POST">
                @csrf

                <div class="input-group mb-3">
                    <span class="input-group-text" id="basic-addon1">@</span>
                    <input type="text" class="form-control" name="email" placeholder="Email Perusahaan"
                        value="{{ old('email') }}" aria-describedby="basic-addon1" required>
                </div>
                ...
                <div class= "form-check">
                    ...
                    <label class="form-check-label" for="exampleCheck1">Dengan lanjut, Anda setuju pada Ketentuan,
                        Privasi, dan Cookie KoneksiBilitas</label>
                </div>
                <button type="submit" class="btn btn-primary">Sign In</button>

            </form>
            <a href="{{ route('register-perusahaan') }}" class="btn btn-outline-primary">Sign Up</a>
            <p class = "penyedia-kerja">Masuk sebagai Pencari Kerja? <a href="{{ route('login') }}">Pencari kerja</a>
            </p>
        </div>
    </div>
